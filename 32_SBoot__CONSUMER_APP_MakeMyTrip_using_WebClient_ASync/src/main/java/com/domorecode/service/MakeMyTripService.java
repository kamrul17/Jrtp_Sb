...
//	}
//ASYNC--------------------------------------------
	public void getTicketByIdSync(Integer tId) {
	System.out.println("sync method started");
	WebClient webClient = WebClient.create();
	String response = webClient.get().uri(IRCTC_ENDPOINT_GET_TICKET, tId).accept(MediaType.APPLICATION_JSON)
			.retrieve().bodyToMono(String.class).block();//wait for response
	System.out.println(response);
	System.out.println("sync method ended");
}
	public void getTicketByIdAsync(Integer tId) {
		System.out.println("sync method started");
		WebClient webClient = WebClient.create();
		webClient.get().uri(IRCTC_ENDPOINT_GET_TICKET, tId).accept(MediaType.APPLICATION_JSON)
